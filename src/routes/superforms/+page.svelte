<script lang="ts">
  ...
  const { form: eForm, errors: eErrors, enhance: eEnhance, message: eMessage } = superForm<EditUserSchema>(initialEditUserForm, {
    dataType: 'json',
    validators: zodClient(editUserSchema as any) // Type assertion needed for discriminated union in Zod 3.x
  });

  // Local state to persist region-specific data when switching away
  // Initialize with default empty structures or existing data if present
  let regionState = {
  ...
  // Hydrate regionState from initial form data if available
  // This is important if we load the page with existing data
  $effect(() => {
    if ($eForm.region === UserRegion.EU && 'eu' in $eForm) {
       regionState[UserRegion.EU] = { ...regionState[UserRegion.EU], ...$eForm.eu };
    } else if ($eForm.region === UserRegion.US && 'us' in $eForm) {
       regionState[UserRegion.US] = { ...regionState[UserRegion.US], ...$eForm.us };
    } else if ($eForm.region === UserRegion.UK && 'uk' in $eForm) {
       regionState[UserRegion.UK] = { ...regionState[UserRegion.UK], ...$eForm.uk };
    } else if ($eForm.region === UserRegion.Other && 'other' in $eForm) {
       regionState[UserRegion.Other] = { ...regionState[UserRegion.Other], ...$eForm.other };
    }
  });
  ...
  function onRegionChange(event: Event) {
    ...
    const previousRegion = $eForm.region;

    // Save current region state before switching
    if (previousRegion === UserRegion.EU && 'eu' in $eForm) {
        regionState[UserRegion.EU] = { ...$eForm.eu };
    } else if (previousRegion === UserRegion.US && 'us' in $eForm) {
        regionState[UserRegion.US] = { ...$eForm.us };
    } else if (previousRegion === UserRegion.UK && 'uk' in $eForm) {
        regionState[UserRegion.UK] = { ...$eForm.uk };
    } else if (previousRegion === UserRegion.Other && 'other' in $eForm) {
        regionState[UserRegion.Other] = { ...$eForm.other };
    }
    ...
    if (region === UserRegion.EU) {
      $eForm = { ...base, region: UserRegion.EU, eu: { ...regionState[UserRegion.EU] } };
    } else if (region === UserRegion.US) {
      $eForm = { ...base, region: UserRegion.US, us: { ...regionState[UserRegion.US] } };
    } else if (region === UserRegion.UK) {
      $eForm = { ...base, region: UserRegion.UK, uk: { ...regionState[UserRegion.UK] } };
    } else if (region === UserRegion.Other) {
      $eForm = { ...base, region: UserRegion.Other, other: { ...regionState[UserRegion.Other] } };
    }
  }
