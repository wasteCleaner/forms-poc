<script lang="ts">
  ...
  // --- Edit User Form ---
  const { form, errors: eErrors, enhance: eEnhance, message: eMessage } = superForm<EditUserSchema>(initialEditUserForm, {
    dataType: 'json',
    validators: zodClient(editUserSchema)
  });

  // Cast form to Writable<EditUserFormState> to allow access to all potential region fields
      ...
      <div class="border-t pt-4">
        ...
        <div class="mt-4 p-4 bg-gray-50 rounded">
            {#if $eForm.region === UserRegion.EU}
                    <!-- Using non-null assertions because we ensure structure in onRegionChange -->
                    <div class="space-y-2">
                        <label class="flex items-center space-x-2">
