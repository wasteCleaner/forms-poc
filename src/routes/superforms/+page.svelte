<script lang="ts">
  ...
  import { type LoginSchema, type EditUserSchema, loginSchema, editUserSchema } from '$lib/schemas';
  ...
  // --- Edit User Form ---
  // Cast schema to any for discriminated unions to avoid shape errors
  // Cast store to Writable<EditUserFormState> to handle all potential fields
  const { form: eFormStore, errors: eErrors, enhance: eEnhance, message: eMessage } = superForm<EditUserSchema>(initialEditUserForm, {
    dataType: 'json',
  ...
  function onRegionChange(event: Event) {
    ...
    const current = $eForm;
    // We need to preserve base fields but switch the discriminated part
    // Using a base object to copy common fields
    const base = {
      email: current.email,
