  ...
  <section class="border p-6 rounded-lg shadow-sm bg-white">
    ...
    <form method="POST" action="?/editUser" use:eEnhance class="space-y-6">
      ...
      <div class="border-t pt-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                 <label for="e-line1" class="block text-sm font-medium">Line 1</label>
                 <input id="e-line1" name="address.line1" bind:value={($eForm as any).address.line1} class="border p-2 w-full rounded" />
                 {#if ($eErrors as any).address?.line1}<span class="text-red-600 text-xs">{($eErrors as any).address.line1}</span>{/if}
            </div>
             <div>
                 <label for="e-city" class="block text-sm font-medium">City</label>
                 <input id="e-city" name="address.city" bind:value={($eForm as any).address.city} class="border p-2 w-full rounded" />
                  {#if ($eErrors as any).address?.city}<span class="text-red-600 text-xs">{($eErrors as any).address.city}</span>{/if}
            </div>
             <div>
                 <label for="e-postalCode" class="block text-sm font-medium">Postal Code</label>
                 <input id="e-postalCode" name="address.postalCode" bind:value={($eForm as any).address.postalCode} class="border p-2 w-full rounded" />
                  {#if ($eErrors as any).address?.postalCode}<span class="text-red-600 text-xs">{($eErrors as any).address.postalCode}</span>{/if}
            </div>
             <div>
                 <label for="e-country" class="block text-sm font-medium">Country</label>
                 <input id="e-country" name="address.country" bind:value={($eForm as any).address.country} class="border p-2 w-full rounded" />
