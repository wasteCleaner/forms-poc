...
export const load: PageServerLoad = async () => {
	const form = (await superValidate(zod(loginSchema as any))) as unknown as SuperValidated<LoginData>;
	return { form };
};

export const actions: Actions = {
	superforms: async ({ request }) => {
		const form = (await superValidate(request, zod(loginSchema as any))) as unknown as SuperValidated<LoginData>;
