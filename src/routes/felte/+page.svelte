<script lang="ts">
  ...
  const errors = eErrors as Readable<Record<string, any>>;

  function onRegionChange(event: Event) {
    const region = (event.target as HTMLSelectElement).value as UserRegion;
    // Update the region field directly
    setFields('region', region);

    // Initialize/Reset fields for the new region, but do NOT set others to undefined
    ...
    <form use:eForm use:enhance method="POST" action="?/editUser" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          {#if $errors.email}<span class="text-red-600 text-xs">{$errors.email}</span>{/if}
        </div>
        ...
        <div>
          ...
          {#if $errors.displayName}<span class="text-red-600 text-xs">{$errors.displayName}</span>{/if}
        </div>
        ...
        <div>
          ...
          <input id="e-locale" type="text" name="locale" class="border p-2 w-full rounded" />
           {#if $errors.locale}<span class="text-red-600 text-xs">{$errors.locale}</span>{/if}
        </div>
      </div>
        ...
        <div class="mt-4 p-4 bg-gray-50 rounded">
            {#if $eData.region === UserRegion.EU}
                <div class="space-y-2">
                    ...
                    {#if $errors.eu?.gdprConsent}<span class="text-red-600 text-xs">{$errors.eu.gdprConsent}</span>{/if}
            ...
            {:else if $eData.region === UserRegion.UK}
                 <div class="space-y-2">
                    ...
                    {#if $errors.uk?.postcode}<span class="text-red-600 text-xs">{$errors.uk.postcode}</span>{/if}
      ...
      <div class="border-t pt-4">
        ...
        <div class="space-y-2">
            {#each $eData.favoriteGames as game, i (game.key || i)}
                ...
                {#if $errors.favoriteGames?.[i]?.id}<span class="text-red-600 text-xs block">{$errors.favoriteGames[i].id}</span>{/if}
            {/each}
        </div>
